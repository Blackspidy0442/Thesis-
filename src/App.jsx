...
const INDICES = [
  { label: "NIFTY 50",   sym: "%5ENSETP",   fallback: 24500 },
  { label: "SENSEX",     sym: "%5EBSESN",   fallback: 80500 },
  { label: "BANK NIFTY", sym: "%5ENSEBANK", fallback: 52000 },
  { label: "NIFTY IT",   sym: "%5ECNXIT",   fallback: 37000 },
  { label: "NIFTY MID",  sym: "%5ENSEI",    fallback: 11200 },
];
...
function useLiveIndices() {
  ...
  const fetchOne = async (idx) => {
    const proxies = [
      `https://api.allorigins.win/get?url=${encodeURIComponent(`https://query1.finance.yahoo.com/v8/finance/chart/${idx.sym}?interval=1d&range=2d`)}`,
      `https://corsproxy.io/?url=${encodeURIComponent(`https://query2.finance.yahoo.com/v8/finance/chart/${idx.sym}?interval=1d&range=2d`)}`,
    ];
...
const IndexTicker = ({ isMobile }) => {
  ...
  const go = async () => { setSpinning(true); await refresh(); setTimeout(() => setSpinning(false), 700); };
    ...
    <div style={{ background: C.surface, borderBottom: `1px solid ${C.border}` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: isMobile ? "3px 14px" : "3px 24px", borderBottom: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <button onClick={go} style={{ background: "none", border: "none", cursor: "pointer", color: C.accent2, fontSize: 14, transition: "transform 0.6s", transform: spinning ? "rotate(360deg)" : "none", display: "inline-block" }}>↻</button>
        </div>
      </div>
      <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch", padding: isMobile ? "8px 14px" : "10px 24px", display: "flex", gap: isMobile ? 22 : 38, scrollbarWidth: "none" }}>
        {data.map((idx) => (
          <div key={idx.label} style={{ flexShrink: 0 }}>
            ...
            {!idx.loading && <div style={{ fontSize: 10, fontFamily: "monospace", marginTop: 1, whiteSpace: "nowrap", color: idx.error ? C.muted : idx.pct >= 0 ? C.greenBright : C.redBright }}>{idx.error ? "retrying···" : `${idx.pct >= 0 ? "▲" : "▼"} ${Math.abs(idx.pct).toFixed(2)}%`}</div>}
          </div>
        ))}
      </div>
    </div>
...
const StatCard = ({ label, value, sub, color, accent = false, small = false }) => (
  <div style={{ background: accent ? "linear-gradient(145deg,#0d0f1a,#0a0c15)" : C.surface, border: `1px solid ${accent ? C.accent+"44" : C.border}`, borderRadius: 10, padding: small ? "13px 14px" : "18px 20px", display: "flex", flexDirection: "column", gap: 4, position: "relative", overflow: "hidden" }}>
...
const MiniChart = ({ trades }) => {
  ...
  const W=400,H=70,p=6, min=Math.min(0,...cum), max=Math.max(...cum), rng=max-min||1;
  const pts = cum.map((v,i)=>`${p+(i/(cum.length-1))*(W-2*p)},${p+(1-(v-min)/rng)*(H-2*p)}`).join(" ");
  const color=cum[cum.length-1]>=0?C.greenBright:C.redBright, ab=p+(1-(0-min)/rng)*(H-2*p), fp=pts.split(" ")[0], lp=pts.split(" ").slice(-1)[0];
  return (
    <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{display:"block"}} preserveAspectRatio="none">
      ...
      <polygon points={`${fp} ${pts} ${lp.split(",")[0]},${ab} ${fp.split(",")[0]},${ab}`} fill="url(#cg)"/>
...
const BarChart = ({ trades }) => {
  ...
  const entries=Object.entries(by).sort((a,b)=>b[1].pnl-a[1].pnl), maxA=Math.max(...entries.map(([,v])=>Math.abs(v.pnl)),1);
  return (
...
export default function Thesis() {
  ...
  // Auto-save trades to localStorage on every change
  useEffect(() => {
  ...
  const stats = useMemo(() => {
    ...
    const net=trades.map(t=>t.pnl-t.fees), wins=net.filter(n=>n>0), losses=net.filter(n=>n<0);
      ...
      <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:isMobile?"0 14px":"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:isMobile?52:60,position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{display:"flex",flexDirection:"column",lineHeight:1}}>
            <span style={{fontFamily:"monospace",fontWeight:700,fontSize:isMobile?15:18,color:C.text}}>Thesis<span style={{color:C.accent}}>.</span></span>
        ...
        {!isMobile&&(
          <div style={{display:"flex",gap:2,background:C.elevated,borderRadius:8,padding:"3px"}}>
            {TABS.map(t=><button key={t} onClick={()=>setTab(t)} style={{padding:"6px 16px",borderRadius:6,border:"none",cursor:"pointer",fontSize:13,fontWeight:500,transition:"all 0.15s",background:tab===t?C.surface:"transparent",color:tab===t?C.text:C.muted,boxShadow:tab===t?"0 1px 4px #0008":"none"}}>{t}</button>)}
          </div>
        )}
        ...
        {tab==="Trades"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
              {["All","Long","Short"].map(d=><button key={d} onClick={()=>setFilterDir(d)} style={{padding:"6px 14px",borderRadius:6,border:`1px solid ${filterDir===d?C.accent:C.border}`,cursor:"pointer",fontSize:12,fontWeight:600,background:filterDir===d?`${C.accent}22`:"transparent",color:filterDir===d?C.accent2:C.muted}}>{d}</button>)}
            ...
            <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:isMobile?14:26,display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:isMobile?10:12}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:5,gridColumn:"span 2"}}>
                  ...
                  <textarea value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))} placeholder="Why did you take this trade?" rows={3} style={{...IS,fontFamily:"sans-serif",fontSize:"15px",resize:"vertical"}}/>
                </div>
              </div>
              ...
              <button onClick={handleSubmit} style={{background:C.accent,border:"none",borderRadius:8,padding:"12px 24px",color:"white",fontFamily:"monospace",fontSize:13,fontWeight:700,cursor:"pointer",touchAction:"manipulation"}}
